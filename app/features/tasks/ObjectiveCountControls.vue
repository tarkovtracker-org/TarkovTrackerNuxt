<template>
  <div class="flex items-center gap-1">
    <div class="flex items-center rounded-md border border-gray-300 bg-white dark:border-white/10 dark:bg-white/5">
      ...
      <div
        v-if="!isEditing"
        class="flex h-7 min-w-14 cursor-pointer items-center justify-center px-2 text-[11px] font-semibold tabular-nums text-gray-900 hover:bg-gray-100 dark:text-gray-100 dark:hover:bg-white/10"
    ...
    <AppTooltip
      ...
      <button
        type="button"
        :disabled="disabled"
        class="cursor-pointer focus-visible:ring-primary-500 focus-visible:ring-offset-surface-900 flex h-7 w-7 items-center justify-center rounded-md border transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60"
        :aria-label="
          currentCount >= neededCount
            ? t('page.tasks.questcard.complete', 'Complete')
            : t('page.tasks.questcard.markComplete', 'Mark complete')
