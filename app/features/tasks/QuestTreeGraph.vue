<template>
  <div class="space-y-4">
    <div class="quest-tree-header flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-white/10 bg-surface-900/70 px-5 py-4">
      <div>
        <p class="text-sm font-semibold uppercase tracking-wider text-white/70">
          {{ t('page.tasks.questtree.title') }}
        </p>
        <p class="text-xs text-white/60">
          {{ t('page.tasks.questtree.legend_context') }}
        </p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="status-pill bg-green-500/20 text-green-300">{{ t('page.tasks.questtree.legend_available') }}</span>
        <span class="status-pill bg-red-500/20 text-red-300">{{ t('page.tasks.questtree.legend_locked') }}</span>
        <span class="status-pill bg-primary-500/20 text-primary-200">{{ t('page.tasks.questtree.legend_completed') }}</span>
      </div>
    </div>
    <div class="quest-tree-canvas rounded-3xl border border-white/5 bg-surface-900/70 p-6">
      <div v-if="!hasNodes" class="flex flex-col items-center gap-3 py-12 text-center text-white/60">
        ...
        <p>{{ t('page.tasks.questtree.empty') }}</p>
      </div>
